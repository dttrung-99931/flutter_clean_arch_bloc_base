...
class Validate {
  ...
  static String? postalCode(String? postalCode) {
    if (isNullOrEmpty(postalCode) || postalCode!.length != /* Constatnts.postalCodeLength */ 8) {
      return 'messages.is_required'.tr(args: ['common.postal_code'.tr()]);
    }
    return null;
  }
  ...
  static String? number(
    ...
    if (isNullOrEmpty(number)) {
      return 'messages.is_required'.tr(args: [fieldName.tr()]);
    }
  ...
  static bool isValidPass(String pass) {
    return RegExp(r'^(?=.*[0-9])(?=.*[a-zA-Z])([a-zA-Z0-9]+){8,16}$').hasMatch(pass);
  }

  static bool isNumeric(String str, {bool? allowDot = true}) {
    if (allowDot! && str.endsWith('.')) {
      return false;
    }
    try {
      double.parse(str);
  ...
  static bool isUuid(String uuid) {
    return RegExp(r'[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}').hasMatch(uuid.toLowerCase());
  }
